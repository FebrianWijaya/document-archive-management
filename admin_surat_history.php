        ...
        if (isset($_POST['upload'])) {
            ...
            if (in_array($uploadedFileType, $allowedFileTypes)) {
                ...
                if ($latestVersion !== null) {
                    // Extract the numeric and alphabetic parts from the version
                    preg_match('/(\d+)\.([a-z])/', $latestVersion, $matches);

                    if (count($matches) === 3) {
                        $numericPart = $matches[1];
                        $alphabeticPart = $matches[2];

                        // Increment the numeric or alphabetic part accordingly
                        if ($alphabeticPart !== 'z') {
                            $version = $numericPart . '.' . chr(ord($alphabeticPart) + 1);
                        } else {
                            // If 'z' is reached, increment the numeric part and reset the alphabetic part
                            $version = ($numericPart + 1) . '.a';
                        }
                    }
                }
                ...
                if ($_FILES['file']['size'] > $maxFileSize) {
                    ...
                    $newFileName = "{$fileType}_{$companyName}_{$uploadDate}_ver.$version.{$fileExtension}";
                    ...
                    if (move_uploaded_file($_FILES['file']['tmp_name'], $uploadPath)) {
                        ...
                        $stmt->bind_param("sssssssss", $folder, $newFileName, $companyName, $uploadDateFormatted, $version, $comment, $division, $fileType, $publisher);                        

                        if ($stmt->execute()) {
                            // Redirect back to history.php
                            header("Location: admin_surat_history.php?folder=" . urlencode($folder));
                            exit;
                        } else {
...
<form action="" method="post" enctype="multipart/form-data">
    ...
    <input type="hidden" name="file_type" value="surat"> <!-- You can set the file type as needed -->
    ...
    <input type="submit" name="upload" value="Upload File">
        ...
        <table border="1">
            ...
            <tbody>
                ...
                $selectFilesQuery = "SELECT file_name, upload_date, publisher, version, comment FROM uploaded_files WHERE folder_name = ? AND division = 'Admin dan Finance' AND file_type = 'surat'";
