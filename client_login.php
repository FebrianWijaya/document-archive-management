...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!isset($_POST['username'], $_POST['password'])) {
        ...
        if ($stmt = $con->prepare('SELECT id, password, perusahaan FROM client_users WHERE username = ?')) {
            ...
            if ($stmt->num_rows > 0) {
                $stmt->bind_result($id, $password, $perusahaan);
                $stmt->fetch();
...
<body>
    <div class="container">
        ...
        <div class="login">
            ...
            <p class="client-login-link">Staff Login: <a href="login.php">Click here</a></p>
        </div>
    </div>
</body>
</html>