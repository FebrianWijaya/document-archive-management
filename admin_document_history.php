...
$allowed_roles = ['Administrator', 'Admin dan Finance'];
                ...
                if ($_FILES['file']['size'] > $maxFileSize) {
                    ...
                    if (move_uploaded_file($_FILES['file']['tmp_name'], $uploadPath)) {
                        ...
                        if ($stmt->execute()) {
                            ...
                            header("Location: admin_document_history.php?folder=" . urlencode($folder));
                            exit;
                        } else {
...
<form action="" method="post" enctype="multipart/form-data">
    ...
    <input type="hidden" name="division" value="Admin dan Finance"> <!-- You can set the division as needed -->
    <input type="hidden" name="file_type" value="Transaksi Perusahaan"> <!-- You can set the file type as needed -->
    ...
    <input type="submit" name="upload" value="Upload File">
        ...
        <table border="1">
            ...
            <tbody>
                ...
                $selectFilesQuery = "SELECT file_name, upload_date, publisher, version, comment FROM uploaded_files WHERE folder_name = ? AND division = 'Admin dan Finance' AND file_type = 'Transaksi Perusahaan'";
                ...
                while ($stmt->fetch()) {
                    ...
                    echo '<a class="download-button" href="download_file.php?folder=' . urlencode($folder) . '&file=' . urlencode($fileName) . '" style="background-color: #007BFF; color: #fff; text-decoration: none; border: none; border-radius: 5px; cursor: pointer; display: inline-block; padding: 5px 10px; margin-right: 5px;margin-left: 5px;">Download</a>|';
                    
                    // Check if the user's role is "Administrator" before displaying the delete button
                    if ($_SESSION['divisi'] === 'Administrator') {
                        echo '<a class="delete-button" href="delete_admin_document.php?folder=' . urlencode($folder) . '&file=' . urlencode($fileName) . '" style="background-color: #FF4D4D; color: #fff; text-decoration: none; border: none; border-radius: 5px; cursor: pointer; display: inline-block; padding: 5px 10px;margin-left: 5px; margin-right: 5px;">Delete</a>';
                    }
                    echo '</td>';
