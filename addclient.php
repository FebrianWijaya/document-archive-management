...
function getNextNoInduk() {
    ...
    $query = "SELECT MAX(no_induk) AS max_no_induk FROM client_users"; // Replace 'your_table_name' with your actual table name
...
<body>
    ...
    <div class="content">
        <h1>Create User</h1>
        <form class="user-form" action="process_create_user_client.php" method="POST">
                ...
                <select id="perusahaan" name="perusahaan" required>
                    ...
                    if ($result && $result->num_rows > 0) {
                        while ($row = $result->fetch_assoc()) {
                            $companyName = htmlspecialchars($row['company_name']); // Sanitize output
