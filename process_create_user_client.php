...
$check_sql = "SELECT * FROM client_users WHERE no_induk = '$no_induk'";
...
$sql = "INSERT INTO client_users (no_induk, nama_depan, nama_belakang, username, password, perusahaan)
        VALUES ('$no_induk', '$nama_depan', '$nama_belakang', '$username', '$password', '$perusahaan')";

if ($conn->query($sql) === TRUE) {
    // JavaScript to show a success pop-up message
    echo '<script>alert("User created successfully!");</script>';
    echo '<script>window.location.href = "client.php";</script>'; // Redirect back to the staff page
    exit(); // Ensure that no further PHP code is executed after the redirect
} else {
